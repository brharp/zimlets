...
function ca_uoguelph_ccs_sidebar () {}

ca_uoguelph_ccs_sidebar.prototype = new ZmZimletBase ();
ca_uoguelph_ccs_sidebar.prototype.constructor = ca_uoguelph_ccs_sidebar;

ca_uoguelph_ccs_sidebar.prototype.dateFormat = new AjxDateFormat ("M/d");
ca_uoguelph_ccs_sidebar.prototype.timeFormat = new AjxDateFormat ("HH:mm");

ca_uoguelph_ccs_sidebar.HELP_URL = 'HelpUrl';

ca_uoguelph_ccs_sidebar.HIDE_PATTERN = /^~/;

ca_uoguelph_ccs_sidebar.APP_MAIL = 'Mail';

ca_uoguelph_ccs_sidebar.APP_CALENDAR = 'Calendar';

ca_uoguelph_ccs_sidebar.SIDEBAR_MENU = 'sidebarMenu';
ca_uoguelph_ccs_sidebar.SIDEBAR_ON   = 'sidebarOn';
ca_uoguelph_ccs_sidebar.SIDEBAR_OFF  = 'sidebarOff';

ca_uoguelph_ccs_sidebar.prototype.getText = function (key)
{
    return this.getMessage(key) || key;
};

ca_uoguelph_ccs_sidebar.prototype.init = function ()
{
    var self = this;

    this.visible = false;
    this.appId = ca_uoguelph_ccs_sidebar.APP_MAIL;
    ...
    this.dwtLabelTop = new ZmAppButton
    ({
        parent: this.dwtRoot,
        id: 'ca_uogelph_ccs_sidebar_tree_top'
    });
    ...
    this.dwtLabelTop.setSize(157,30);
...
ca_uoguelph_ccs_sidebar.prototype.initializeToolbar = function(app, toolbar, controller, viewId) 
{
    if (viewId == ZmId.VIEW_CONVLIST || viewId == ZmId.VIEW_TRAD) {
        ...
        var btnSidebarOn = menu.createMenuItem(ca_uoguelph_ccs_sidebar.SIDEBAR_ON, {
            text:         this.getText("sidebarOn"), 
            radioGroupId: ca_uoguelph_ccs_sidebar.SIDEBAR_MENU, 
            style:        DwtMenuItem.RADIO_STYLE
        });
        btnSidebarOn.addSelectionListener(new AjxListener(this, this.onSidebarMenu, ca_uoguelph_ccs_sidebar.SIDEBAR_ON));
        var btnSidebarOff = menu.createMenuItem(ca_uoguelph_ccs_sidebar.SIDEBAR_OFF, {
            text:         this.getText("sidebarOff"), 
            radioGroupId: ca_uoguelph_ccs_sidebar.SIDEBAR_MENU,
            style:        DwtMenuItem.RADIO_STYLE
        });
        btnSidebarOff.addSelectionListener(new AjxListener(this, this.onSidebarMenu, ca_uoguelph_ccs_sidebar.SIDEBAR_OFF));
    }
};

ca_uoguelph_ccs_sidebar.prototype.onSidebarMenu = function(action) 
{
    this.setShow(ca_uoguelph_ccs_sidebar.SIDEBAR_ON === action);
};

ca_uoguelph_ccs_sidebar.prototype.getShowPref = function (treeItem)
{
    return ((this.getUserProperty(treeItem.getData("show_pref")) ||
             treeItem.getData("show_pref_default")) != "false");
};

ca_uoguelph_ccs_sidebar.prototype.onTreeEvent = function (ev)
{
...
ca_uoguelph_ccs_sidebar.prototype.onSelectApp = function (id)
{
    this.setAppId (id);
};

ca_uoguelph_ccs_sidebar.prototype.updateTreeItem = function (parent,
    startTime,
    endTime,
    showDate)
{
    ...
    for (var i = 0; i < events.size(); i++) {
        ...
        if (folder.getName().match(ca_uoguelph_ccs_sidebar.HIDE_PATTERN)) {
            continue;
        }
...
ca_uoguelph_ccs_sidebar.prototype.collectNewsFolders = function (list, root)
{
    if (!root)
    {
        return list;
    ...
    if (root.type == ZmOrganizer.FOLDER && root.url
        && !root.getName().match(ca_uoguelph_ccs_sidebar.HIDE_PATTERN))
        {
...
ca_uoguelph_ccs_sidebar.prototype.updateNews = function ()
{
...
ca_uoguelph_ccs_sidebar.prototype.errorCallback = function ()
{
    // do nothing
    };

ca_uoguelph_ccs_sidebar.prototype.showNewsItems = function (items, result)
{
...
ca_uoguelph_ccs_sidebar.prototype.updateTasks = function ()
{
    // create the json object for the search request
    var jsonObj = {
        SearchRequest:{
...
ca_uoguelph_ccs_sidebar.prototype.showTasks = function (items, result)
{
...
ca_uoguelph_ccs_sidebar.prototype.showEvent = function (event)
{
    var ctrl = AjxDispatcher.run("GetCalController");
    ctrl.show();
    ctrl._viewMgr.setDate(event.startDate, 0, true);
    this.setAppId(ca_uoguelph_ccs_sidebar.APP_CALENDAR);
};

ca_uoguelph_ccs_sidebar.prototype.showNewsItem = function (item)
{
    var ctrl = AjxDispatcher.run("GetMsgController");
    ctrl.show (new ZmMailMsg(item.id), null, null, true);
};

ca_uoguelph_ccs_sidebar.prototype.update = function ()
{
...
ca_uoguelph_ccs_sidebar.prototype.setVisible = function (visible) {
    ...
    /* Update menu */
    if (this.dwtMenu) {
        this.dwtMenu.getMenuItem(ca_uoguelph_ccs_sidebar.SIDEBAR_ON).setChecked(visible, true);
        this.dwtMenu.getMenuItem(ca_uoguelph_ccs_sidebar.SIDEBAR_OFF).setChecked(!visible, true);
    }
};

ca_uoguelph_ccs_sidebar.prototype.setShow = function (show) {
    this.show[this.appId] = show;
    this.update();
    this.notify();
};

ca_uoguelph_ccs_sidebar.prototype.setAppId = function (id)
{
    if (this.appId != id)
    {
        this.appId = id;
...
ca_uoguelph_ccs_sidebar.prototype.notify = function ()
{
    this.setVisible (this.show[this.appId] == true);
};

ca_uoguelph_ccs_sidebar.prototype.resize = function ()
{
    var rect;
    if (this.visible) {
...
ca_uoguelph_ccs_sidebar.prototype.showHelp = function () {
    window.open(this.getText(ca_uoguelph_ccs_sidebar.HELP_URL));
};
